...
#define HEARTBEAT_PROMPT \
    "Read " FEMTO_HEARTBEAT_FILE " and follow any instructions or tasks listed there. " \
...
{
    FILE *f = fopen(FEMTO_HEARTBEAT_FILE, "r");
    if (!f) {
        return false;
    }
...
static bool heartbeat_send(void)
{
    ...
    strncpy(msg.channel, FEMTO_CHAN_SYSTEM, sizeof(msg.channel) - 1);
...
esp_err_t heartbeat_init(void)
{
    ESP_LOGI(TAG, "Heartbeat service initialized (file: %s, interval: %ds)",
             FEMTO_HEARTBEAT_FILE, FEMTO_HEARTBEAT_INTERVAL_MS / 1000);
    return ESP_OK;
}
...
esp_err_t heartbeat_start(void)
{
    ...
    s_heartbeat_timer = xTimerCreate(
        "heartbeat",
        pdMS_TO_TICKS(FEMTO_HEARTBEAT_INTERVAL_MS),
        pdTRUE,    /* auto-reload */
        NULL,
    ...
    ESP_LOGI(TAG, "Heartbeat started (every %d min)", FEMTO_HEARTBEAT_INTERVAL_MS / 60000);
    return ESP_OK;
}
