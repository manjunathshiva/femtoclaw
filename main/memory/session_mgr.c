...
{
    snprintf(buf, size, "%s/tg_%s.jsonl", FEMTO_SPIFFS_SESSION_DIR, chat_id);
}

esp_err_t session_mgr_init(void)
{
    ESP_LOGI(TAG, "Session manager initialized at %s", FEMTO_SPIFFS_SESSION_DIR);
    return ESP_OK;
}
...
esp_err_t session_get_history_json(const char *chat_id, char *buf, size_t size, int max_msgs)
{
    ...
    cJSON *messages[FEMTO_SESSION_MAX_MSGS];
    int count = 0;
    int write_idx = 0;
...
{
    DIR *dir = opendir(FEMTO_SPIFFS_SESSION_DIR);
    if (!dir) {
        /* SPIFFS is flat, so list all files matching pattern */
        dir = opendir(FEMTO_SPIFFS_BASE);
        if (!dir) {
