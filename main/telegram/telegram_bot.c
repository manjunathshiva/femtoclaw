...
static char s_bot_token[128] = FEMTO_SECRET_TG_TOKEN;
...
static void save_update_offset_if_needed(bool force)
{
    ...
    nvs_handle_t nvs;
    if (nvs_open(FEMTO_NVS_TG, NVS_READWRITE, &nvs) != ESP_OK) {
        return;
    }
...
static char *tg_api_call_via_proxy(const char *path, const char *post_data)
{
    proxy_conn_t *conn = proxy_conn_open("api.telegram.org", 443,
                                          (FEMTO_TG_POLL_TIMEOUT_S + 5) * 1000);
    if (!conn) return NULL;
    ...
    int timeout = (FEMTO_TG_POLL_TIMEOUT_S + 5) * 1000;
    while (1) {
        if (len + 1024 >= cap) {
            cap *= 2;
...
static char *tg_api_call_direct(const char *method, const char *post_data)
{
    ...
    esp_http_client_config_t config = {
        ...
        .user_data = &resp,
        .timeout_ms = (FEMTO_TG_POLL_TIMEOUT_S + 5) * 1000,
        .buffer_size = 2048,
        .buffer_size_tx = 2048,
...
static void process_updates(const char *json_str)
{
    ...
    cJSON_ArrayForEach(update, result) {
        ...
        /* Push to inbound bus */
        femto_msg_t msg = {0};
        strncpy(msg.channel, FEMTO_CHAN_TELEGRAM, sizeof(msg.channel) - 1);
...
static void telegram_poll_task(void *arg)
{
    ...
    while (1) {
        ...
        snprintf(params, sizeof(params),
                 ...
                 s_update_offset, FEMTO_TG_POLL_TIMEOUT_S);
...
esp_err_t telegram_bot_init(void)
{
    ...
    nvs_handle_t nvs;
    if (nvs_open(FEMTO_NVS_TG, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[128] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_TG_TOKEN, tmp, &len) == ESP_OK && tmp[0]) {
    ...
    /* s_bot_token is already initialized from FEMTO_SECRET_TG_TOKEN as fallback */
...
esp_err_t telegram_bot_start(void)
{
    BaseType_t ret = xTaskCreatePinnedToCore(
        ...
        FEMTO_TG_POLL_STACK, NULL,
        FEMTO_TG_POLL_PRIO, NULL, FEMTO_TG_POLL_CORE);

    return (ret == pdPASS) ? ESP_OK : ESP_FAIL;
}

/* Sanitize a string in-place: replace invalid UTF-8 bytes with '?' */
...
esp_err_t telegram_send_message(const char *chat_id, const char *text)
{
    ...
    /* Sanitize: replace invalid UTF-8 bytes so Telegram won't reject */
    char *safe_text = strdup(text);
    if (!safe_text) return ESP_ERR_NO_MEM;
    sanitize_utf8(safe_text);
    ...
    while (offset < text_len) {
        ...
        if (chunk > FEMTO_TG_MAX_MSG_LEN) {
            chunk = FEMTO_TG_MAX_MSG_LEN;
            /* Back up to a UTF-8 character boundary to avoid splitting
             * multi-byte characters (emoji, CJK, etc.) across chunks */
            while (chunk > 0 && (safe_text[offset + chunk] & 0xC0) == 0x80) {
                chunk--;
            }
        }
        ...
        memcpy(segment, safe_text + offset, chunk);
        segment[chunk] = '\0';
        ...
        if (!sent_ok) {
            ...
            if (seg2) {
                memcpy(seg2, safe_text + offset, chunk);
                seg2[chunk] = '\0';
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_TG, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_TG_TOKEN, token));
