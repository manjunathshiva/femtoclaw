...
static void outbound_dispatch_task(void *arg)
{
    ...
    while (1) {
        ...
        if (strcmp(msg.channel, FEMTO_CHAN_TELEGRAM) == 0) {
        ...
        } else if (strcmp(msg.channel, FEMTO_CHAN_WEBSOCKET) == 0) {
            esp_err_t ws_err = ws_server_send(msg.chat_id, msg.content);
            if (ws_err != ESP_OK) {
                ESP_LOGW(TAG, "WS send failed for %s: %s", msg.chat_id, esp_err_to_name(ws_err));
            }
        } else if (strcmp(msg.channel, FEMTO_CHAN_SYSTEM) == 0) {
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "  FemtoClaw - ESP32-S3 AI Agent");
    ESP_LOGI(TAG, "========================================");
    ...
    if (wifi_err == ESP_OK) {
        ...
        if (wifi_manager_wait_connected(30000) == ESP_OK) {
            ...
            ESP_LOGW(TAG, "WiFi connection timeout. Check FEMTO_SECRET_WIFI_SSID in femto_secrets.h");
        }
    } else {
        ESP_LOGW(TAG, "No WiFi credentials. Set FEMTO_SECRET_WIFI_SSID in femto_secrets.h");
    }

    ESP_LOGI(TAG, "FemtoClaw ready. Type 'help' for CLI commands.");
}
