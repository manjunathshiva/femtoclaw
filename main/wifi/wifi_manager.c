                          ...
                          int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_count < FEMTO_WIFI_MAX_RETRY) {
            /* Exponential backoff: 1s, 2s, 4s, 8s, ... capped at 30s */
            uint32_t delay_ms = FEMTO_WIFI_RETRY_BASE_MS << s_retry_count;
            if (delay_ms > FEMTO_WIFI_RETRY_MAX_MS) {
                delay_ms = FEMTO_WIFI_RETRY_MAX_MS;
            }
            ESP_LOGW(TAG, "Disconnected, retry %d/%d in %" PRIu32 "ms",
                     s_retry_count + 1, FEMTO_WIFI_MAX_RETRY, delay_ms);
            vTaskDelay(pdMS_TO_TICKS(delay_ms));
            esp_wifi_connect();
            s_retry_count++;
        } else {
            ESP_LOGE(TAG, "Failed to connect after %d retries", FEMTO_WIFI_MAX_RETRY);
...
esp_err_t wifi_manager_start(void)
{
    ...
    nvs_handle_t nvs;
    if (nvs_open(FEMTO_NVS_WIFI, NVS_READONLY, &nvs) == ESP_OK) {
        size_t len = sizeof(wifi_cfg.sta.ssid);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_SSID, (char *)wifi_cfg.sta.ssid, &len) == ESP_OK) {
            len = sizeof(wifi_cfg.sta.password);
            nvs_get_str(nvs, FEMTO_NVS_KEY_PASS, (char *)wifi_cfg.sta.password, &len);
            found = true;
        }
        nvs_close(nvs);
    }
    ...
    if (!found) {
        if (FEMTO_SECRET_WIFI_SSID[0] != '\0') {
            strncpy((char *)wifi_cfg.sta.ssid, FEMTO_SECRET_WIFI_SSID, sizeof(wifi_cfg.sta.ssid) - 1);
            strncpy((char *)wifi_cfg.sta.password, FEMTO_SECRET_WIFI_PASS, sizeof(wifi_cfg.sta.password) - 1);
            found = true;
        }
    }
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_WIFI, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_SSID, ssid));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_PASS, password));
