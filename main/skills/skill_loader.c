...
#define BUILTIN_SKILL_CREATOR \
    "# Skill Creator\n" \
    "\n" \
    "Create new skills for FemtoClaw.\n" \
    "\n" \
    "## When to use\n" \
...
{
    char path[64];
    snprintf(path, sizeof(path), "%s%s.md", FEMTO_SKILLS_PREFIX, skill->filename);
...
{
    DIR *dir = opendir(FEMTO_SPIFFS_BASE);
    if (!dir) {
    ...
    while ((ent = readdir(dir)) != NULL && off < size - 1) {
        ...
        /* Build full path */
        char full_path[296];
        snprintf(full_path, sizeof(full_path), "%s/%s", FEMTO_SPIFFS_BASE, name);
