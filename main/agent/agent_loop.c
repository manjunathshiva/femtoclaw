...
static void append_turn_context_prompt(char *prompt, size_t size, const femto_msg_t *msg)
{
...
static char *patch_tool_input_with_context(const llm_tool_call_t *call, const femto_msg_t *msg)
{
    ...
    if (channel && strcmp(channel, FEMTO_CHAN_TELEGRAM) == 0 &&
        strcmp(msg->channel, FEMTO_CHAN_TELEGRAM) == 0 && msg->chat_id[0] != '\0') {
...
static cJSON *build_tool_results(const llm_response_t *resp, const femto_msg_t *msg,
...
static void agent_loop_task(void *arg)
{
    ...
    char *system_prompt = femto_alloc(FEMTO_CONTEXT_BUF_SIZE);
    char *history_json = femto_alloc(FEMTO_LLM_STREAM_BUF_SIZE);
    char *tool_output = femto_alloc(TOOL_OUTPUT_SIZE);
    ...
    while (1) {
        ...
        context_build_system_prompt(system_prompt, FEMTO_CONTEXT_BUF_SIZE);
        append_turn_context_prompt(system_prompt, FEMTO_CONTEXT_BUF_SIZE, &msg);
        ...
        session_get_history_json(msg.chat_id, history_json,
                                 FEMTO_LLM_STREAM_BUF_SIZE, FEMTO_AGENT_MAX_HISTORY);
        ...
        while (iteration < FEMTO_AGENT_MAX_TOOL_ITER) {
            /* Send "working" indicator before each API call */
#if FEMTO_AGENT_SEND_WORKING_STATUS
            if (!sent_working_status && strcmp(msg.channel, FEMTO_CHAN_SYSTEM) != 0) {
                femto_msg_t status = {0};
                strncpy(status.channel, msg.channel, sizeof(status.channel) - 1);
                strncpy(status.chat_id, msg.chat_id, sizeof(status.chat_id) - 1);
                status.content = strdup("femto is working...");
                if (status.content) {
...
esp_err_t agent_loop_start(void)
{
    ...
    for (size_t i = 0; i < (sizeof(stack_candidates) / sizeof(stack_candidates[0])); i++) {
        ...
        BaseType_t ret = xTaskCreatePinnedToCore(
            ...
            stack_size, NULL,
            FEMTO_AGENT_PRIO, NULL, FEMTO_AGENT_CORE);
