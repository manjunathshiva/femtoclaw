...
static char s_model[LLM_MODEL_MAX_LEN] = FEMTO_LLM_DEFAULT_MODEL;
static char s_provider[16] = FEMTO_LLM_PROVIDER_DEFAULT;

#define LLM_BASE_URL_MAX_LEN 256
static char s_api_base_url[LLM_BASE_URL_MAX_LEN] = {0};  /* custom base URL (e.g. LM Studio, Ollama) */
...
#if FEMTO_LLM_LOG_VERBOSE_PAYLOAD
...
#else
    if (FEMTO_LLM_LOG_PREVIEW_BYTES > 0) {
        size_t shown = total > FEMTO_LLM_LOG_PREVIEW_BYTES ? FEMTO_LLM_LOG_PREVIEW_BYTES : total;
        char preview[FEMTO_LLM_LOG_PREVIEW_BYTES + 1];
...
static const char *llm_api_url(void)
{
    ...
    return provider_is_openai() ? FEMTO_OPENAI_API_URL : FEMTO_LLM_API_URL;
}
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (FEMTO_SECRET_API_KEY[0] != '\0') {
        safe_copy(s_api_key, sizeof(s_api_key), FEMTO_SECRET_API_KEY);
    }
    if (FEMTO_SECRET_MODEL[0] != '\0') {
        safe_copy(s_model, sizeof(s_model), FEMTO_SECRET_MODEL);
    }
    if (FEMTO_SECRET_MODEL_PROVIDER[0] != '\0') {
        safe_copy(s_provider, sizeof(s_provider), FEMTO_SECRET_MODEL_PROVIDER);
    }
    if (FEMTO_SECRET_API_BASE_URL[0] != '\0') {
        safe_copy(s_api_base_url, sizeof(s_api_base_url), FEMTO_SECRET_API_BASE_URL);
    }

    /* NVS overrides take highest priority (set via CLI) */
    nvs_handle_t nvs;
    if (nvs_open(FEMTO_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[LLM_API_KEY_MAX_LEN] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_API_KEY, tmp, &len) == ESP_OK && tmp[0]) {
            safe_copy(s_api_key, sizeof(s_api_key), tmp);
        }
        char model_tmp[LLM_MODEL_MAX_LEN] = {0};
        len = sizeof(model_tmp);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_MODEL, model_tmp, &len) == ESP_OK && model_tmp[0]) {
            safe_copy(s_model, sizeof(s_model), model_tmp);
        }
        char provider_tmp[16] = {0};
        len = sizeof(provider_tmp);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_PROVIDER, provider_tmp, &len) == ESP_OK && provider_tmp[0]) {
            safe_copy(s_provider, sizeof(s_provider), provider_tmp);
        }
        char base_url_tmp[LLM_BASE_URL_MAX_LEN] = {0};
        len = sizeof(base_url_tmp);
        if (nvs_get_str(nvs, FEMTO_NVS_KEY_API_BASE_URL, base_url_tmp, &len) == ESP_OK && base_url_tmp[0]) {
            safe_copy(s_api_base_url, sizeof(s_api_base_url), base_url_tmp);
        }
        nvs_close(nvs);
    }
    ...
    if (s_api_key[0]) {
        if (s_api_base_url[0]) {
            ESP_LOGI(TAG, "LLM proxy initialized (provider: %s, model: %s, base_url: %s)",
                     s_provider, s_model, s_api_base_url);
        } else {
            ESP_LOGI(TAG, "LLM proxy initialized (provider: %s, model: %s)", s_provider, s_model);
        }
    } else if (s_api_base_url[0]) {
        ESP_LOGI(TAG, "LLM proxy initialized (provider: %s, model: %s, base_url: %s, no API key)",
                 s_provider, s_model, s_api_base_url);
    } else {
...
static esp_err_t llm_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    if (provider_is_openai()) {
        ...
        esp_http_client_set_header(client, "anthropic-version", FEMTO_LLM_API_VERSION);
    }
    ...
    if (provider_is_openai()) {
        ...
        if (s_api_key[0]) {
            hlen = snprintf(header, sizeof(header),
                ...
                "Content-Length: %d\r\n"
                "Connection: close\r\n\r\n",
                llm_api_path(), llm_api_host(), s_api_key, FEMTO_LLM_API_VERSION, body_len);
        } else {
            hlen = snprintf(header, sizeof(header),
                "POST %s HTTP/1.1\r\n"
                "Host: %s\r\n"
                ...
                "Content-Length: %d\r\n"
                "Connection: close\r\n\r\n",
                llm_api_path(), llm_api_host(), FEMTO_LLM_API_VERSION, body_len);
        }
    }
                         ...
                         llm_response_t *resp)
{
    ...
    if (s_api_key[0] == '\0' && s_api_base_url[0] == '\0') return ESP_ERR_INVALID_STATE;

    /* Build request body (non-streaming) */
    cJSON *body = cJSON_CreateObject();
    cJSON_AddStringToObject(body, "model", s_model);
    if (provider_is_openai()) {
        cJSON_AddNumberToObject(body, "max_completion_tokens", FEMTO_LLM_MAX_TOKENS);
    } else {
        cJSON_AddNumberToObject(body, "max_tokens", FEMTO_LLM_MAX_TOKENS);
    }
    ...
    /* HTTP call */
    resp_buf_t rb;
    if (resp_buf_init(&rb, FEMTO_LLM_STREAM_BUF_SIZE) != ESP_OK) {
        free(post_data);
            ...
            if (message) {
                ...
                if (tool_calls && cJSON_IsArray(tool_calls)) {
                    ...
                    cJSON_ArrayForEach(tc, tool_calls) {
                        if (resp->call_count >= FEMTO_MAX_TOOL_CALLS) break;
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_API_KEY, api_key));
...
esp_err_t llm_set_model(const char *model)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_MODEL, model));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_provider(const char *provider)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_PROVIDER, provider));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_api_base_url(const char *url)
{
    ...
    ESP_ERROR_CHECK(nvs_open(FEMTO_NVS_LLM, NVS_READWRITE, &nvs));
    if (url && url[0]) {
        ESP_ERROR_CHECK(nvs_set_str(nvs, FEMTO_NVS_KEY_API_BASE_URL, url));
    } else {
        nvs_erase_key(nvs, FEMTO_NVS_KEY_API_BASE_URL);
    }
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);

    safe_copy(s_api_base_url, sizeof(s_api_base_url), url ? url : "");
    if (s_api_base_url[0]) {
        ESP_LOGI(TAG, "API base URL set to: %s", s_api_base_url);
    } else {
        ESP_LOGI(TAG, "API base URL cleared (using default)");
    }
    return ESP_OK;
}
