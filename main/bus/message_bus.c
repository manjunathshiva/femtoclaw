...
{
    s_inbound_queue = xQueueCreate(FEMTO_BUS_QUEUE_LEN, sizeof(femto_msg_t));
    s_outbound_queue = xQueueCreate(FEMTO_BUS_QUEUE_LEN, sizeof(femto_msg_t));
    ...
    ESP_LOGI(TAG, "Message bus initialized (queue depth %d)", FEMTO_BUS_QUEUE_LEN);
    return ESP_OK;
}

esp_err_t message_bus_push_inbound(const femto_msg_t *msg)
{
...
esp_err_t message_bus_pop_inbound(femto_msg_t *msg, uint32_t timeout_ms)
{
...
esp_err_t message_bus_push_outbound(const femto_msg_t *msg)
{
...
esp_err_t message_bus_pop_outbound(femto_msg_t *msg, uint32_t timeout_ms)
{
    TickType_t ticks = (timeout_ms == UINT32_MAX) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
