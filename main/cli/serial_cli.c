...
/* --- set_api_base_url command --- */
static struct {
    struct arg_str *url;
    struct arg_end *end;
} api_base_url_args;

static int cmd_set_api_base_url(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&api_base_url_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, api_base_url_args.end, argv[0]);
        return 1;
    }
    llm_set_api_base_url(api_base_url_args.url->sval[0]);
    printf("API base URL set.\n");
    return 0;
}

/* --- clear_api_base_url command --- */
static int cmd_clear_api_base_url(int argc, char **argv)
{
    llm_set_api_base_url("");
    printf("API base URL cleared. Default provider URLs will be used.\n");
    return 0;
}
...
static int cmd_config_show(int argc, char **argv)
{
    ...
    print_config("WiFi SSID",  FEMTO_NVS_WIFI,   FEMTO_NVS_KEY_SSID,     FEMTO_SECRET_WIFI_SSID,  false);
    print_config("WiFi Pass",  FEMTO_NVS_WIFI,   FEMTO_NVS_KEY_PASS,     FEMTO_SECRET_WIFI_PASS,  true);
    print_config("TG Token",   FEMTO_NVS_TG,     FEMTO_NVS_KEY_TG_TOKEN, FEMTO_SECRET_TG_TOKEN,   true);
    print_config("API Key",    FEMTO_NVS_LLM,    FEMTO_NVS_KEY_API_KEY,  FEMTO_SECRET_API_KEY,    true);
    print_config("Model",      FEMTO_NVS_LLM,    FEMTO_NVS_KEY_MODEL,    FEMTO_SECRET_MODEL,      false);
    print_config("Provider",   FEMTO_NVS_LLM,    FEMTO_NVS_KEY_PROVIDER, FEMTO_SECRET_MODEL_PROVIDER, false);
    print_config("Base URL",   FEMTO_NVS_LLM,    FEMTO_NVS_KEY_API_BASE_URL, FEMTO_SECRET_API_BASE_URL, false);
    print_config("Proxy Host", FEMTO_NVS_PROXY,  FEMTO_NVS_KEY_PROXY_HOST, FEMTO_SECRET_PROXY_HOST, false);
    print_config("Proxy Port", FEMTO_NVS_PROXY,  FEMTO_NVS_KEY_PROXY_PORT, FEMTO_SECRET_PROXY_PORT, false);
    print_config("Search Key", FEMTO_NVS_SEARCH, FEMTO_NVS_KEY_API_KEY,  FEMTO_SECRET_SEARCH_KEY, true);
    printf("=============================\n");
    return 0;
}
...
static int cmd_config_reset(int argc, char **argv)
{
    const char *namespaces[] = {
        FEMTO_NVS_WIFI, FEMTO_NVS_TG, FEMTO_NVS_LLM, FEMTO_NVS_PROXY, FEMTO_NVS_SEARCH
    };
    for (int i = 0; i < 5; i++) {
        nvs_handle_t nvs;
...
esp_err_t serial_cli_init(void)
{
    ...
    esp_console_cmd_t model_cmd = {
        .command = "set_model",
        .help = "Set LLM model (default: " FEMTO_LLM_DEFAULT_MODEL ")",
        .func = &cmd_set_model,
        .argtable = &model_args,
    };
    ...
    esp_console_cmd_t provider_cmd = {
        ...
        .help = "Set LLM model provider (default: " FEMTO_LLM_PROVIDER_DEFAULT ")",
    ...
    /* set_api_base_url */
    api_base_url_args.url = arg_str1(NULL, NULL, "<url>", "API base URL (e.g. http://192.168.1.100:1234)");
    api_base_url_args.end = arg_end(1);
    esp_console_cmd_t api_base_url_cmd = {
        .command = "set_api_base_url",
        .help = "Set custom API base URL for LM Studio/Ollama (use with provider openai)",
        .func = &cmd_set_api_base_url,
        .argtable = &api_base_url_args,
    };
    esp_console_cmd_register(&api_base_url_cmd);

    /* clear_api_base_url */
    esp_console_cmd_t clear_base_url_cmd = {
        .command = "clear_api_base_url",
        .help = "Clear custom API base URL, revert to default provider URLs",
        .func = &cmd_clear_api_base_url,
    };
    esp_console_cmd_register(&clear_base_url_cmd);
