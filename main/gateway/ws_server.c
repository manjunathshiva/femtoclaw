...
static ws_client_t s_clients[FEMTO_WS_MAX_CLIENTS];

static ws_client_t *find_client_by_fd(int fd)
{
    for (int i = 0; i < FEMTO_WS_MAX_CLIENTS; i++) {
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    if (type && cJSON_IsString(type) && strcmp(type->valuestring, "message") == 0
        ...
        /* Push to inbound bus */
        femto_msg_t msg = {0};
        strncpy(msg.channel, FEMTO_CHAN_WEBSOCKET, sizeof(msg.channel) - 1);
...
esp_err_t ws_server_start(void)
{
    ...
    config.server_port = FEMTO_WS_PORT;
    config.ctrl_port = FEMTO_WS_PORT + 1;
    config.max_open_sockets = FEMTO_WS_MAX_CLIENTS;
