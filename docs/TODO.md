# FemtoClaw vs Nanobot — Feature Gap Tracker

> Comparing against `nanobot/` reference implementation. Tracks features FemtoClaw has not yet aligned with.
...
- **FemtoClaw**: `memory_write_long_term` and `memory_append_today` exist but are only called from CLI; agent loop never writes memory
- **Scope**: Expose `memory_write` and `memory_append_today` as tool_use tools for Claude; add system prompt guidance on when to persist memory; optionally add pre-compaction flush (trigger memory save when session history nears `FEMTO_SESSION_MAX_MSGS`)
...
- **FemtoClaw**: No authentication; anyone can message the bot and consume API credits
- **Recommendation**: Store allow_from list in `femto_secrets.h` as a build-time define, filter in `process_updates()`
...
- **FemtoClaw**: Uses `parse_mode: Markdown` directly; special characters can cause send failures (has fallback to plain text)
...
- **FemtoClaw**: Not handled; /start is sent to Claude as a regular message
...
- **FemtoClaw**: Only processes `message.text`, ignores all media messages
...
- **FemtoClaw**: Only loads `SOUL.md` and `USER.md`
...
- **FemtoClaw**: `context_builder.c` only reads last 3 days
...
- **FemtoClaw**: `femto_msg_t` only has channel + chat_id + content
...
- **FemtoClaw**: Hardcoded if-else dispatch
...
- **FemtoClaw**: Hardcoded to Anthropic Messages API
...
- Implemented: `femto_secrets.h` as build-time defaults, NVS as runtime override via CLI
...
- **FemtoClaw**: Basic JSON protocol, lacks streaming token push
...
- **FemtoClaw**: Hardcoded in app_main()
...
- **FemtoClaw**: Only Telegram + WebSocket
- **Recommendation**: Low priority, Telegram is sufficient

### [x] ~~Telegram Proxy Support (HTTP CONNECT)~~
- Implemented: HTTP CONNECT tunnel via `proxy/http_proxy.c`, configurable via `femto_secrets.h` (`FEMTO_SECRET_PROXY_HOST`/`FEMTO_SECRET_PROXY_PORT`)

### [ ] Session Metadata Persistence
- **nanobot**: `session/manager.py` L136-153 — session file includes metadata line (created_at, updated_at)
- **FemtoClaw**: JSONL only stores role/content/ts, no metadata header
...
- [x] SPIFFS storage
- [x] Build-time config (`femto_secrets.h`) + runtime NVS override via CLI
